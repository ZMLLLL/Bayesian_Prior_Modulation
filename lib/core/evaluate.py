...
class LossVector(object):
    def __init__(self, num_classes):
        self.num_classes = num_classes
        self.reset()

    def reset(self):
        self.loss_vecotor = np.zeros((self.num_classes, ), dtype=float)
        self.count = np.zeros((self.num_classes, ), dtype=int)

    def update(self, loss, label):
        self.count[label] += 1
        self.loss_vecotor[label] += loss

    def get_avg_loss_per_class(self):
        rec = np.array(
            [
                self.loss_vecotor[i] / self.count[i]
                 for i in range(self.num_classes)
            ]
        )
        rec[np.isnan(rec)] = 0
